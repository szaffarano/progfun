...
class Empty extends TweetSet {
  ...
  def mostRetweeted: Tweet = throw new NoSuchElementException("There is not most retweeted Tweet in an empty set")
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def mostRetweeted: Tweet = {
    ...
    try {
      ...
      if(rightMost.retweets > temp.retweets) rightMost else temp
    } catch {
      case _: NoSuchElementException => temp
    }
  }
...
object GoogleVsApple {
  ...
  lazy val googleTweets: TweetSet = TweetReader.allTweets.filterAcc(
    t => google.exists(s => t.text.contains(s)), new Empty)
  lazy val appleTweets: TweetSet = TweetReader.allTweets.filterAcc(
    t => google.exists(s => t.text.contains(s)), new Empty)
